...
export class RBACSeeder {
  ...
  private static async seedRolePermissions(db: any) {
    ...
    for (const [roleName, permissionNames] of Object.entries(rolePermissionMap)) {
      ...
      await rolePermissionRepo.createQueryBuilder()
        .delete()
  ...
  private static async seedUsers(db: any) {
    ...
    const defaultUsers = [
      ...
      { username: 'supervisor', password: 'super123', fullName: 'Supervisor User', roleName: 'Supervisor' },
      { username: 'operator', password: 'oper123', fullName: 'Operator User', roleName: 'Operator' },
      { username: 'viewer', password: 'view123', fullName: 'Viewer User', roleName: 'Viewer' }
    ]

    for (const userData of defaultUsers) {
      let existing = await userRepo.findOne({ where: { username: userData.username } })
