...
export const getLocalStorage = (nameSpace: string): { data: any; type: string } | null => {
  if (typeof window === 'undefined') return null;
  const data = localStorage.getItem(nameSpace);
  if (!data) return { data: null, type: 'null' };
  return JSON.parse(data);
};

export const setLocalStorage = (nameSpace: string, value: any) => {
...
export const removeLocalStorage = (nameSpace: string) => {
  if (typeof window === 'undefined') return;
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(LOGIN_STORE)) {
        const loginStore = db.createObjectStore(LOGIN_STORE, {
          keyPath: 'id',
          autoIncrement: true,
        });
