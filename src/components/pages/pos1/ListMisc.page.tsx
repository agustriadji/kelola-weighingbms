...
import FormMiscPage from '@/components/pages/pos1/FormMisc.page';
import { useState } from 'react';
import { useForm } from 'react-hook-form';
import * as Dialog from '@radix-ui/react-dialog';

export default function ListMiscPage({ data, refresh }: { data: any[]; refresh: () => void }) {
  const [showModal, setShowModal] = useState(false);
  const { control } = useForm();
  const propertiesText = {
    headTitle: 'List Miscellaneous',
    buttonCreateText: `+ Create`,
    buttonViewText: `View`,
    modalHeadText: 'Create Miscellaneous',
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <Dialog.Root open={showModal} onOpenChange={setShowModal}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto z-50">
